...
项目测试已按三层拆分，目的是让运营调参更安全：

- `config-contract`：检查配置合法性（字段、类型、概率范围、表达式安全）
- `engine-behavior`：检查流程行为（开局、修炼、结算、事件触发）是否正常
- `pinned-values`：只检查少量固定常量（如默认存档结构）

这意味着：

- 你改数值（例如 `baseChance`、事件 `chance`、属性加减）通常不需要同步改测试。
- 只有配置格式不合法、或改动导致流程行为异常时，测试才会失败。

出现测试失败时，先按下面顺序看：

1. 是否有语法问题（漏逗号、引号、括号）。
2. `chance` 是否仍在 `0~1`。
3. `trigger/condition/effects` 是否保持声明式结构（不要写函数）。
