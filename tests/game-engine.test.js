...
test('draw then redraw keeps redraw button hidden on second draw (when test mode off)', () => {
  const { game, getElementById, config } = createEnvironment();

  // 关闭测试模式以验证原有行为
  config.rules.debug = config.rules.debug || {};
  config.rules.debug.enableTalentTest = false;
...
test('draw then redraw keeps redraw button visible when test mode on', () => {
  const { game, getElementById, config } = createEnvironment();

  // 开启测试模式
  config.rules.debug = config.rules.debug || {};
  config.rules.debug.enableTalentTest = true;
...
test('getQiyunExemptionThreshold calculates correctly', () => {
  ...
  // 公式：(境界 * 2 + 3) * 10
  assert.strictEqual(game.getQiyunExemptionThreshold(0), 30);   // Peasant
  assert.strictEqual(game.getQiyunExemptionThreshold(1), 50);   // Page
  assert.strictEqual(game.getQiyunExemptionThreshold(2), 70);   // Squire
  assert.strictEqual(game.getQiyunExemptionThreshold(9), 210);  // Prince
});
...
test('positive event gets qiyun bonus on chance', () => {
  ...
  const event = { text: 'test positive event', chance: baseChance, effects: [{ field: 'cultivation', add: 100 }] };
...
test('negative and death events do not get qiyun bonus on chance', () => {
  ...
  const negativeEvent = { text: 'test negative', chance: baseChance, isNegative: true, effects: [{ field: 'cultivation', add: -100 }] };
  const deathEvent = { text: 'test death', chance: baseChance, isDeath: true };
...
test('event cooldown prevents same event within 10 years', () => {
  const { game } = createEnvironment();
  const eventText = 'test cooldown event';
